    ...
    <form>
      <div class="form-row">
        ...
        <div class="col">
          ...
          <input type="number" class="form-control" v-model="config.port" id="inputPort" placeholder="1433" />
        </div>
      </div>
      <div class="form-row">
        <div class="col">
...
export default {
  ...
  mounted: function () {
    if (this.msconfig.host !== undefined) {
      this.verifyConnection()
    }
  },
  data () {
    return {
      config: this.msconfig,
  ...
  methods: {
    verifyConnection: function () {
      ...
      (new ConnectionApiHelper()).verifyConnection('mssql', this.config).then(rsp => {
        this.loading = false
        if (rsp.status === 200) {
