...
package com.hagoapp.datacova.surveyor.datetime
        ...
        fun stringToDateTimeOrNull(input: String, extraFormats: Set<String> = emptySet()): ZonedDateTime? {
            ...
            for (dtFmt in dateTimeFormatters.plus(extraFormats.map { DateTimeFormatter.ofPattern(it) })) {
                try {
                    ...
                    logger.debug("stringToDateTimeOrNull parsing error DateTimeParseException {} using {}", ex, dtFmt)
                } catch (ex: DateTimeException) {
                    logger.debug("stringToDateTimeOrNull parsing error DateTimeException {} using {}", ex, dtFmt)
                }
            }
            return d
        }
        ...
        fun stringToDateOrNull(input: String, extraFormats: Set<String> = emptySet()): LocalDate? {
            ...
            for (dtFmt in dateFormatters.plus(extraFormats.map { DateTimeFormatter.ofPattern(it) })) {
                try {
                    ...
                    logger.debug("stringToDateOrNull parsing error DateTimeParseException {} using {}", ex, dtFmt)
                } catch (ex: IllegalArgumentException) {
                    logger.debug("stringToDateOrNull parsing error IllegalArgumentException {} using {}", ex, dtFmt)
                } catch (ex: DateTimeException) {
                    logger.debug("stringToDateOrNull parsing error DateTimeException {} using {}", ex, dtFmt)
                }
            }
            return d
        }
        ...
        fun stringToTimeOrNull(input: String, extraFormats: Set<String> = emptySet()): LocalTime? {
            ...
            for (dtFmt in timeFormatters.plus(extraFormats.map { DateTimeFormatter.ofPattern(it) })) {
                try {
                    ...
                    logger.debug("stringToTimeOrNull parsing error DateTimeParseException {} using {}", ex, dtFmt)
                } catch (ex: IllegalArgumentException) {
                    logger.debug("stringToTimeOrNull parsing error IllegalArgumentException {} using {}", ex, dtFmt)
                } catch (ex: DateTimeException) {
                    logger.debug("stringToTimeOrNull parsing error DateTimeException {} using {}", ex, dtFmt)
                }
            }
            return d
        }
